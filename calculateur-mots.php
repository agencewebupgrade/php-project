...
function calculateurMots($chaineCaractere) {//on définit la fonction qui calcule le nombre de mots
    ...
    return 'Il y a '.$numberOfWords.' mots dans la phrase "'.$_POST['string'].'"'; //on retourne une phrase avec le nombre de mots et la phrase d'origine
}

if (isset($_POST['string']) && !empty($_POST['string'])) {//on vérifie qu'une phrase a bien été renseignée
    echo calculateurMots($_POST['string']);//on affiche le retour de notre fonction
}else{
